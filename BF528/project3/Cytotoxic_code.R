...
# sample information
info <- read.csv("toxgroup_1_rna_infos3.csv")
...
write.csv(res,'deseq_results_cytotoxic.csv')
# write out matrix of normalized counts
write.csv(counts(dds,normalized=TRUE),'deseq_norm_counts_cytotoxic.csv')
#ds3<-read.csv("deseq_results.csv",sep=',')
res[order(res$padj),]
head(res,c=10)
sigDE <- subset(res,padj <0.05)
sigDE <-head(sigDE,n=100)
write.csv(sigDE,'top100_cytotoxic.csv')
dim(sigDE)
head(sigDE,c=10)
hist(sigDE$log2FoldChange,breaks=100,col="blue",main="Cytotoxic",xlab="log2FoldChange")

plot(-log(sigDE$pvalue)~sigDE$log2FoldChange,pch=20,col="red")
