ds1<- read.csv("sample1_n.csv",sep=',')
...
# sample information
info <- read.csv("toxgroup_1_rna_infos1.csv")
...
write.csv(res,'deseq_results_ahr.csv')
# write out matrix of normalized counts
write.csv(counts(dds,normalized=TRUE),'deseq_norm_counts_ahr.csv')
...
write.csv(sigDE,'top100_ahr.csv')

hist(sigDE$log2FoldChange,breaks=100,col="blue",main="Ahr",xlab="log2FoldChange")

plot(-log(sigDE$pvalue)~sigDE$log2FoldChange,pch=20,col="red",main="Ahr",xlab="log2FoldChange")
